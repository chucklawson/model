// ============================================
// FILE: src/Components/CustomRangePerformanceModal/CustomRangeLineChart.tsx
// Custom Range Portfolio Growth Line Chart
// ============================================
...
interface CustomRangeLineChartProps {
  customRangeData: DateRangePortfolioPerformance;
}

export default function CustomRangeLineChart({ customRangeData }: CustomRangeLineChartProps) {
  const chartData = useMemo(() => {
    return customRangeData.dailyPortfolioValues.map(day => ({
      date: formatDateShort(day.date),
      fullDate: day.date,
      value: day.totalValue,
      baseline: customRangeData.totalBaselineValue
    }));
  }, [customRangeData]);

  const isPositive = customRangeData.totalRangeGainDollar >= 0;
    ...
    <div className="bg-slate-50 p-6 rounded-lg">
      ...
      <ResponsiveContainer width="100%" height={400}>
        <ComposedChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 60 }}>
          ...
          <ReferenceLine
            y={customRangeData.totalBaselineValue}
            stroke="#94a3b8"
            strokeDasharray="5 5"
            label={{
              value: 'Baseline',
