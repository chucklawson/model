...
export async function callFmpApi<T = any>(options: FmpApiClientOptions): Promise<T> {
...
export async function getUserFmpApiKey() {
  const { data } = await amplifyClient.models.FmpApiKey.list();
...
export async function setUserFmpApiKey(apiKey: string) {
  ...
  if (existing) {
    return await amplifyClient.models.FmpApiKey.update({
      id: existing.id,
      apiKey,
      isActive: true,
    ...
    return await amplifyClient.models.FmpApiKey.create({
      apiKey,
      isActive: true,
...
export async function deleteUserFmpApiKey() {
  const existing = await getUserFmpApiKey();
  if (existing) {
    return await amplifyClient.models.FmpApiKey.delete({ id: existing.id });
  }
}
