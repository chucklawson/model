// ============================================
// Browser-compatible Vanguard PDF Parser
// ============================================
import { PDFParse } from 'pdf-parse';
import { generateVanguardCSV } from './vanguardCsvGenerator';
import type { VanguardPdfTransaction, VanguardPdfData } from './vanguardPdfTypes';

// Configure PDF.js worker using PDFParse's setWorker method
// Use unpkg CDN for the worker file to avoid bundling issues
PDFParse.setWorker('https://unpkg.com/pdfjs-dist@5.4.296/build/pdf.worker.mjs');
...
/**
 * Parse Vanguard PDF from a File object (browser compatible)
 *
 * @param file - PDF File object from file input
 * @returns Parsed account number and transactions
 */
export async function parseVanguardPdfFromFile(file: File): Promise<VanguardPdfData> {
...
/**
 * Convert Vanguard PDF to CSV (browser compatible)
 *
 * @param file - PDF File object from file input
 * @returns CSV content as string
 */
export async function convertVanguardPdfToCsv(file: File): Promise<string> {
  // Parse PDF
  const pdfData = await parseVanguardPdfFromFile(file);

  // Generate CSV
  const csvContent = generateVanguardCSV(pdfData.accountNumber, pdfData.transactions);
